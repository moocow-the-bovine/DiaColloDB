...
our %EXPORT_TAGS =
    (
     ...
     pdl   => [qw(_intersect_p _union_p _complement_p _setdiff_p),
	       qw(readPdlFile writePdlFile mmzeroes)
	      ],
    );
...
##  + create a temporary mmap()ed pdl using mapfraw(); %opts:
##    (
...
sub mmzeroes {
  ...
  #require PDL::IO::FastRaw;
  my $that = UNIVERSAL::isa($_[0],__PACKAGE__) ? shift : __PACKAGE__;
  my $obj  = UNIVERSAL::isa($_[0],'PDL') ? shift : undef;
  my $file = !ref($_[0]) && $_[0] !~ /^[0-9]$/ ? shift : undef;
  my $type = UNIVERSAL::isa($_[0],'PDL::Type') ? shift : (defined($obj) ? $obj->type : PDL::double());
  my $opts = UNIVERSAL::isa($_[$#_],'HASH') ? pop(@_) : {};
  my @dims = defined($obj) ? $obj->dims : @_;
  ...
  push(@MMZTMP,$file) if ($temp);

  ##-- remove any stale data
  mmunlink($file);

  ##-- guts
  $that->vlog($opts->{log}, "mmzeroes: $type \[".join(',',@dims)."\] $file ".($temp ? "(TEMP)" : "(KEEP)"));
  return PDL::IO::FastRaw::mapfraw($file, {Dims=>\@dims, Datatype=>$type, Creat=>1});
}
...
sub mmunlink {
  ...
  foreach (grep {-e "$file$_"} ('','.hdr')) {
    CORE::unlink("$file$_") or $that->logwarn("mmunlink(): failed to unlink tempfile '$file$_': $!");
  }
  return 1;
}
